...
| 비용 | 호출당 4~6초, 캐시 읽기 11k + 생성 4~8k 토큰. Claude Code 시스템 프롬프트가 실리기 때문이고 줄일 수 없다 |
...
실제 호출이라 케이스당 몇 초와 토큰이 든다. 프롬프트를 고쳤을 때 돌리면 된다 — 조용히 나빠질 수 있는 것이 그것뿐이다. 각 도구의 읽기 형태와 쓰기 형태를 짝지어 두었다. 질문이 "docker가 안전한가"가 아니라 "서브커맨드를 봤는가"이기 때문이다. 마지막 측정은 36개 중 36개 정확, 호출당 평균 5.0초였다.
