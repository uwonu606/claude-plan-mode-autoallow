...
**위 표가 증명하는 범위는 훅이 무엇을 결정했는가까지다.** 그 결정이 화면에서 어떻게 보였는지 — 프롬프트가 떴는지 — 는 들어 있지 않다. 에이전트 쪽에서는 관측할 수 없고(도구 결과만 보면 "프롬프트 없이 통과"와 "프롬프트가 떠서 승인됨"이 똑같이 생겼다), 세션 트랜스크립트에도 권한 판정 레코드가 없다. 위쪽 항목들이 "프롬프트가 떴다"고 단정할 수 있는 이유는 사람이 그 자리에서 보고 적었기 때문이다.

네 번째 건에서 그 차이가 실제로 문제가 됐다. 훅이 승인하지 않았는데 명령이 실행됐고 사용자는 프롬프트를 보지 못했다. 원인을 추적하다 **세션 자체가 `auto` 모드였다**는 것이 드러났다 — 트랜스크립트의 모드 스냅샷을 보면 세션 대부분이 `auto`이고 `plan`은 검증을 위해 진입한 짧은 구간뿐이었다. 훅의 페이로드에 `permission_mode: plan`이 온 것은 로그가 증명하므로 그 구간이 plan이었던 것은 맞지만, 아래 항목이 경고하는 조건 위에서 잰 셈이 됐다.

**그래서 이 절의 네 번째 행은 "분류기가 거부했고 훅이 승인하지 않았다"까지만 유효하다.** 그 뒤에 무엇이 그 명령을 실행시켰는지는 이 측정으로 답할 수 없다. 훅이 침묵한 뒤의 동작을 보려는 것이므로, 바탕이 `auto`이면 볼 수 있는 것이 없다.

다시 잴 사람을 위해 함정 하나. **`/permissions`로는 모드가 바뀌지 않는다** — 그 화면은 allow/deny 규칙을 관리하는 곳이고 모드는 `Shift+Tab` 순환이나 `--permission-mode` 플래그가 정한다. `Shift+Tab`도 순환에 `auto`가 들어 있어 한 바퀴 돌아 제자리로 오기 쉽다. 확실한 방법은 `claude --permission-mode default`로 새 세션을 띄우는 것이다. 위 네 건은 이 함정에 걸려 세 번 모두 `auto` 위에서 측정됐고, 세 번 모두 같은 결과가 나왔다.
