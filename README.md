...
**그래서 이 절의 네 번째 행은 "분류기가 거부했고 훅이 승인하지 않았다"까지만 유효하다.** 그 뒤에 무엇이 그 명령을 실행시켰는지는 이 측정으로 답할 수 없다. 다시 재려면 세션의 바탕 모드를 `default`로 두고 plan mode에 들어가야 한다. 훅이 침묵한 뒤의 동작을 보려는 것이므로, 바탕이 `auto`이면 볼 수 있는 것이 없다.
